...
if [ -b $1 ]; then
  ...
  if [ "$1" == "" ]; then
    ...
    image="$(pwd)/rpi_ubuntu_${deb_arch}_${deb_release}_${mydate}.img"
...
apt-get -y install locales console-common ntp openssh-server less vim parted sudo
useradd pi -s /bin/bash -m -G sudo
echo "raspberry" | passwd "pi" --stdin
