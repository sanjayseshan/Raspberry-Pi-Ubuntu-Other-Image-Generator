...
if [ -b $1 ]; then
  ...
  if [ "$1" == "" ]; then
    ...
    image="$(pwd)/ev3_linux_${deb_arch}_${deb_release}_${mydate}.img"
...
LANG=C chroot $rootfs "echo 'root:mindstorms' | chpasswd"
chroot $rootfs
mkdir -p boot/flash
#cp -rvp 
mount $bootp $bootfs
