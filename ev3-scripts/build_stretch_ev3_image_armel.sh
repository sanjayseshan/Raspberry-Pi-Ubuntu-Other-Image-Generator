...
debootstrap --foreign --arch $deb_arch jessie $rootfs $deb_mirror
...
echo "deb http://archive.ev3dev.org/debian jessie main contrib non-free
" >> etc/apt/sources.list
mkdir -p boot/flash
...
apt-get -y install git-core binutils ca-certificates wget curl sudo
...
apt-get -f install
apt-get clean
apt-get install ev3dev-base bluez
apt-get -d install brickman
rm -f cleanup
service ntp stop
service ssh stop
